...
define('WFTP_DEBUG', false); // Set to true for detailed debugging logs
...
function wftp_add_admin_menu() {
    add_menu_page(
        ...
        'manage_options', // Capacidade alterada para manage_options
        'woo-ftp-importer',
        'wftp_admin_page',
        'dashicons-upload',
        30 // Posição ajustada para aparecer mais alto no menu
    );
}
...
function wftp_admin_page() {
    ...
    if (isset($_POST['wftp_save_settings']) && wp_verify_nonce($_POST['wftp_nonce'], 'wftp_save_settings')) {
        ...
        $debug_mode = isset($_POST['wftp_debug_mode']) ? 1 : 0;
        update_option('wftp_debug_mode', $debug_mode);
    ...
    if (isset($_POST['wftp_clear_cache']) && wp_verify_nonce($_POST['wftp_cache_nonce'], 'wftp_clear_cache')) {
        delete_option('wftp_processed_files');
        echo '<div class="notice notice-success is-dismissible"><p>' . __('File processing cache cleared. All files will be reprocessed on next scan.', 'woo-ftp-importer') . '</p></div>';
    }
    ...
    $debug_mode = get_option('wftp_debug_mode', 0);
    ...
    <div class="wrap">
        ...
        <form method="post" action="">
            ...
            <table class="form-table">
                ...
                <tr valign="top">
                    <th scope="row">
                        <label for="wftp_debug_mode"><?php _e('Debug Mode', 'woo-ftp-importer'); ?></label>
                    </th>
                    <td>
                        <input type="checkbox" id="wftp_debug_mode" name="wftp_debug_mode" value="1" <?php checked($debug_mode, 1); ?> />
                        <p class="description">
                            <?php _e('Enable detailed debugging information in logs', 'woo-ftp-importer'); ?>
                        </p>
                    </td>
                </tr>
            </table>
        ...
        <div class="wftp-scan-status" style="margin-bottom: 20px; padding: 15px; background: #f0f0f1; border-left: 4px solid #2271b1;" data-last-scan="<?php echo esc_attr($last_scan); ?>">
            ...
            <p><strong><?php _e('Tracked files:', 'woo-ftp-importer'); ?></strong> <?php echo esc_html($processed_count); ?></p>
        ...
        <div class="wftp-actions" style="display: flex; gap: 10px; margin-bottom: 20px;">
            ...
            <form method="post" action="">
                <?php wp_nonce_field('wftp_clear_cache', 'wftp_cache_nonce'); ?>
                <input type="submit" name="wftp_clear_cache" class="button-secondary" value="<?php _e('Clear File Processing Cache', 'woo-ftp-importer'); ?>" onclick="return confirm('<?php _e('Are you sure? This will cause all files to be reprocessed on next scan.', 'woo-ftp-importer'); ?>');" />
            </form>
        </div>
 ...
 */
function wftp_add_log($message, $is_debug = false) {
    if ($is_debug && !get_option('wftp_debug_mode', 0)) {
        return; // Skip debug messages unless debug mode is enabled
    }
 ...
 * @return string MD5 hash or empty string on error
 */
function wftp_get_file_hash($file_path) {
    if (!file_exists($file_path)) {
        return '';
    }
    
    // For files larger than 5MB, use a different approach to avoid memory issues
    $filesize = filesize($file_path);
    if ($filesize > 5 * 1024 * 1024) {
        // For large files, hash only the first 1MB and file size
        $handle = fopen($file_path, 'rb');
        $data = fread($handle, 1024 * 1024); // First MB
        fclose($handle);
        return md5($data . $filesize);
    } else {
    ...
    try {
        ...
        foreach ($folder_mappings as $folder_name => $user_id) {
            ...
            foreach ($files as $file) {
                ...
                // Add to current files list with file hash
                $file_hash = wftp_get_file_hash($file_path);
                $current_files[$product_key] = array(
                    'path' => $file_path,
                    'hash' => $file_hash
                );
                
                // Process the file
                $result = wftp_process_file($file, $file_path, $user_id, $file_hash);
                ...
                if ($result === 'created') {
                    ...
                    $total_skipped++;
                    if ($debug_mode) {
                        wftp_add_log("Skipped unchanged file: $file (user: $user_id)", true);
                    }
                }
            }
        }
        ...
        wftp_add_log("Scan completed. Created: $total_created, Updated: $total_updated, Skipped: $total_skipped products");
    } catch (Exception $e) {
...
function wftp_remove_deleted_products($current_files, $processed_files) {
    ...
    foreach ($processed_files as $product_key => $file_info) {
 ...
 * @param string $file_hash MD5 hash of file contents
 * @return string 'created', 'updated', 'skipped', or 'error'
 */
function wftp_process_file($filename, $file_path, $user_id, $file_hash = '') {
    // Get the modification time of the file
    $file_mod_time = filemtime($file_path);
    
    // If no hash was provided, calculate one
    if (empty($file_hash)) {
        $file_hash = wftp_get_file_hash($file_path);
    }
    ...
    if (isset($processed_files[$product_key])) {
        // Previous processing info exists
        $previous_hash = isset($processed_files[$product_key]['hash']) ? 
                        $processed_files[$product_key]['hash'] : '';
        
        // Skip if the file hasn't changed (same hash)
        if ($previous_hash === $file_hash && !empty($file_hash)) {
            if ($debug_mode) {
                wftp_add_log("Skipping file, hash unchanged: $filename", true);
            }
            return 'skipped';
        }
        
        // For legacy data without hash, use timestamp
        if (empty($previous_hash) && is_numeric($processed_files[$product_key])) {
            $last_processed = $processed_files[$product_key];
            if ($last_processed >= $file_mod_time) {
                if ($debug_mode) {
                    wftp_add_log("Skipping file, timestamp unchanged: $filename", true);
                }
                return 'skipped';
            }
        }
    ...
    if ($debug_mode) {
        wftp_add_log("Processing file: $filename with hash: $file_hash", true);
    }
    ...
    if ($existing_product_query->have_posts()) {
        ...
        if ($product) {
            ...
            // Update the processed files record with hash
            $processed_files[$product_key] = array(
                'timestamp' => time(),
                'hash' => $file_hash,
