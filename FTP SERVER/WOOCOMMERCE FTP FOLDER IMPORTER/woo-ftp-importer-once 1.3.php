...
/**
 * Generate a preview with watermark for a file
 */
function wftp_generate_preview_with_watermark($file_path) {
    $output_path = str_replace(array('.jpg', '.jpeg', '.png', '.gif', '.webp'), '-preview$0', $file_path);
    $watermark_text = 'PREVIEW';

    list($image_width, $image_height, $image_type) = getimagesize($file_path);
    $image = null;
    switch ($image_type) {
        case IMAGETYPE_JPEG:
 ...
 */
function wftp_set_preview_with_watermark($product_id, $file_path) {
    $preview_path = wftp_generate_preview_with_watermark($file_path);

    if (file_exists($preview_path)) {
        // Set product image
        wftp_update_product_image($product_id, $preview_path, basename($preview_path));
    }
}

/**
 * Display product preview with watermark
 */
function wftp_display_product_preview() {
    global $product;
    $product_id = $product->get_id();
    $source_file = get_post_meta($product_id, '_wftp_source_file', true);
    if ($source_file) {
        $preview_path = wftp_generate_preview_with_watermark($source_file);
        if (file_exists($preview_path)) {
            if (wp_check_filetype($preview_path)['ext'] === 'mkv') {
                echo '<video controls><source src="' . esc_url($preview_path) . '" type="video/mp4">Your browser does not support the video tag.</video>';
            } else {
                echo '<img src="' . esc_url($preview_path) . '" alt="Product Preview">';
            }
        }
    }
}
add_action('woocommerce_before_single_product_summary', 'wftp_display_product_preview', 20);

// Ensure PHP GD extension is installed for watermarking functionality
if (!extension_loaded('gd')) {
    add_action('admin_notices', function() {
        echo '<div class="error"><p>' . __('WooCommerce FTP Folder Importer requires the PHP GD extension to be installed and enabled on your server.', 'woo-ftp-importer') . '</p></div>';
    });
    return;
}

// Ensure your font file path is correct in the wftp_generate_preview_with_watermark function
$font_path = __DIR__ . '/arial.ttf';
if (!file_exists($font_path)) {
    add_action('admin_notices', function() {
        echo '<div class="error"><p>' . __('WooCommerce FTP Folder Importer requires the arial.ttf font file to be placed in the plugin directory.', 'woo-ftp-importer') . '</p></div>';
    });
    return;
}