...
function wsftp_admin_page()
{
    ...
    <div class="wrap">
        ...
        <div class="notice notice-info">
            <p><strong>Create Once Mode:</strong> In this mode, the plugin will only create products for files it has never
                seen before.
        ...
        <form method="post" action="">
            ...
            <table class="form-table">
                ...
                <tr valign="top" class="auth-key">
                    ...
                    <td>
                        <input type="text" id="wsftp_sftp_private_key_path" name="wsftp_sftp_private_key_path"
                            class="regular-text" value="<?php echo esc_attr($sftp_private_key_path); ?>"
            ...
            <table class="form-table">
                ...
                <tr valign="top">
                    ...
                    <td>
                        <input type="checkbox" id="wsftp_remove_deleted_files" name="wsftp_remove_deleted_files" value="1"
                            <?php checked($remove_deleted_files, 1); ?> />
                        <p class="description">
        ...
        <div class="wsftp-scan-status"
            style="margin-bottom: 20px; padding: 15px; background: #f0f0f1; border-left: 4px solid #2271b1;"
            data-last-scan="<?php echo esc_attr($last_scan); ?>">
            ...
            <p><strong>Files already processed:</strong> <?php echo esc_html($processed_count); ?> (these will not create
                new products)</p>
            <p>The scanner will automatically run once every <?php echo esc_html($scan_interval); ?> minute(s) when your
                site receives visitors.</p>
        </div>
        ...
        <div class="wsftp-actions" style="display: flex; gap: 10px; margin-bottom: 20px;">
            ...
            <form method="post" action="">
                ...
                <a href="<?php echo esc_url(admin_url('admin.php?page=woo-sftp-importer&wsftp_fix_categories=1')); ?>"
                    class="button-secondary" style="background-color:#46b450;border-color:#2e8540;color:#fff;"
                    title="Use this to update all product categories based on their SFTP folder">
...
function wsftp_scan_folders()
{
    ...
    try {
        ...
        // Clear folder mappings cache to ensure fresh data
...
function wsftp_create_product($filename, $file_path, $user_id, $product_key)
{
    ...
    // Extrair o nome da pasta SFTP original do product_key
    // O product_key é formado por user_id-filename, precisamos obter o nome da pasta do SFTP que contém o arquivo
    $original_folder = '';

    // Tenta encontrar a pasta SFTP original
    $folders = explode('-', $product_key, 2);
    if (count($folders) > 1) {
        // O primeiro valor é o user_id, vamos buscar o nome da pasta SFTP associada a este usuário
        $user_folders = wsftp_get_sftp_folders();
        foreach ($user_folders as $folder_name => $folder_user_id) {
    ...
    // Se não encontramos a pasta, usamos um valor padrão
    if (empty($original_folder)) {
        $original_folder = 'SFTP Products';
    }

    // Create or get the category with the folder name
    $category = get_term_by('name', $original_folder, 'product_cat');
    if (!$category) {
        // Create the category if it doesn't exist
        $category_id = wp_insert_term($original_folder, 'product_cat');
        if (is_wp_error($category_id)) {
            wsftp_add_log("Error creating category for folder: $original_folder");
            $category_id = 0;
        } else {
    ...
    $product->update_meta_data('_wsftp_source_folder', $original_folder);
    ...
    if ($product_id) {
        if ($is_image) {
            ...
            // Add action hook for watermark processing
            do_action('wsftp_after_product_created', $product_id, $file_path, $filename);
        }
...
/**
 * Get all SFTP folders and their user mappings - caches the result for performance
 */
function wsftp_get_sftp_folders()
{
    static $folder_mappings = null;

    // Return cached result if available
    if ($folder_mappings !== null) {
        return $folder_mappings;
    }

    // Try to get from transient cache
    $folder_mappings = get_transient('wsftp_folder_mappings');
    if ($folder_mappings !== false) {
        return $folder_mappings;
    }

    // If not in cache, fetch from SFTP
    $connection = wsftp_connect_to_sftp();
    if (!$connection) {
        wsftp_add_log("Failed to connect to SFTP server when getting folder mappings.");
        return array();
    }
    ...
    // Cache for 1 hour
    set_transient('wsftp_folder_mappings', $folder_mappings, HOUR_IN_SECONDS);
...
// Set up scan scheduling
add_action('init', function () {
    // Schedule evento com frequência personalizada
    if (!wp_next_scheduled('wsftp_scan_hook')) {
        wp_schedule_event(time(), 'wsftp_custom_interval', 'wsftp_scan_hook');
    }
});

// Adicionar intervalo personalizado de minutos
add_filter('cron_schedules', function ($schedules) {
    $interval = get_option('wsftp_scan_interval', 1);
    $interval = max(1, intval($interval)); // Garantir que seja pelo menos 1 minuto

    $schedules['wsftp_custom_interval'] = array(
        'interval' => $interval * 60,
        'display' => sprintf(__('Every %d minutes', 'woo-sftp-importer'), $interval)
    );
...
add_action('wp', function () {
    // Verificar se precisa executar um scan
    $last_scan = get_option('wsftp_last_scan_time', 0);
    $interval = get_option('wsftp_scan_interval', 1); // em minutos

    if (time() - $last_scan >= ($interval * 60)) {
        // Programar o scan para execução imediata
        if (!wp_next_scheduled('wsftp_delayed_scan_hook')) {
...
// Para reaplicar a categoria nos produtos existentes
add_action('admin_init', function () {
    if (isset($_GET['wsftp_fix_categories']) && current_user_can('manage_options')) {
        wsftp_fix_all_product_categories();
        wp_redirect(admin_url('admin.php?page=woo-sftp-importer&categories_fixed=true'));
        exit;
    }

    if (isset($_GET['categories_fixed'])) {
        add_action('admin_notices', function () {
            echo '<div class="notice notice-success is-dismissible"><p>Todas as categorias de produtos foram atualizadas com base nas pastas do SFTP.</p></div>';
        });
    }
});

/**
 * Fix categories for all existing products
 */
function wsftp_fix_all_product_categories()
{
    ...
    foreach ($products as $product_id) {
        ...
        // Encontrar a pasta SFTP associada ao usuário
        ...
        if (empty($folder_name)) {
            $folder_name = 'SFTP Products';
        }

        // Criar ou obter categoria
        $category = get_term_by('name', $folder_name, 'product_cat');
        if (!$category) {
            $category_data = wp_insert_term($folder_name, 'product_cat');
        ...
        // Salvar nome da pasta para referência futura
        update_post_meta($product_id, '_wsftp_source_folder', $folder_name);

        $count++;
    }

    wsftp_add_log("Categorias de produtos atualizadas: $count produtos processados");
    return $count;
}
