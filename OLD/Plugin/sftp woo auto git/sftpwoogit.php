<?php
/**
 * Plugin Name: SFTP to WooCommerce - Auto
 * Description: Converte arquivos SFTP em produtos WooCommerce com monitoramento automático
 * Version: 1.2
 * Author: DevSpacek
 */
...
class SFTP_To_Woo_Auto {
    ...
    public function activate_cron() {
        if (!wp_next_scheduled('sftp_auto_scan_hook')) {
            ...
            wp_schedule_event(time(), 'hourly', 'sftp_auto_scan_hook');
    ...
    public function render_admin_page() {
        ...
        $auto_frequency = get_option('sftp_auto_frequency', 'hourly');
        ...
        ?>
        <div class="wrap">
            <h1>SFTP para WooCommerce (Automático)</h1>
    ...
    public function maybe_reschedule_cron($old_value, $new_value) {
        ...
        if (get_option('sftp_auto_enabled', 'yes') === 'yes') {
            $frequency = get_option('sftp_auto_frequency', 'hourly');
